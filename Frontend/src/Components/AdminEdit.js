...
const handleSaveName = async () => {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          if (latestImage) {
            ...
            setadminImage(latestImage);  // Set the image URL to adminImage
            setPreview(latestImage);       // Set the preview to the latest image URL
          } else {
            setadminImage("assets/images/avatar/07.jpg");  // Fallback to default if no image
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        const latestImageUrl = `http://localhost:9000/public/AdminPhotos/${res.data.imageFile}`;
        // Store the image URL in localStorage
        localStorage.setItem("adminImage", latestImageUrl);
        setadminImage(latestImageUrl);  // Persist image in adminImage state
  ...
  const handleReset = () => {
    setFile(null);
    setPreview(adminImage); // Reset to current profile image, not default
  };
