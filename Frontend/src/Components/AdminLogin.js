...
const AdminLogin = () => {
  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await axios.post('http://13.232.236.57:3000/send-otp', { email });
  ...
  const verifyOtp = async () => {
    try {
      const response = await axios.post('http://13.232.236.57:3000/verify-otp', { email, userOtp: otp });
      setMessage(response.data.message);
