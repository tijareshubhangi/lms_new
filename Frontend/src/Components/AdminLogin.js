...
const AdminLogin = () => {
  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await axios.post('http://43.204.234.11:3000/send-otp', { email });
  ...
  const verifyOtp = async () => {
    try {
      const response = await axios.post('http://43.204.234.11:3000/verify-otp', { email, userOtp: otp });
      setMessage(response.data.message);
